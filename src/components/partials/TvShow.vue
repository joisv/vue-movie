...
import getMovies from '../../composable/fetch'
    ...
    const { getAiringToday, airingToday, baseImgUrl } = getMovies()
    onMounted(() => {
        getAiringToday()
...
<template>
    <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-2 p-2 max-w-screen-xl m-auto">
       <template v-for="tv in airingToday.results" :key="tv.id">
        <div class="min-h-[160px] w-full cursor-pointer">
            <RouterLink :to="{name : 'tvdetail' , params: { id: tv.id } }">
            <div class="relative flex flex-wrap items-center justify-center after:w-full after:h-full after:hover:bg-bgDark after:opacity-25 after:absolute after:z-30 after:top-0 after:ease-in after:duration-75">
                <img class="object-cover object-center" :src="`${baseImgUrl}${tv.poster_path}`" alt="">
                <div class="h-fit w-auto p-1 bg-primary text-xs rounded-full absolute top-1 right-1 font-medium text-center">8.2</div>
                <div class="absolute">
            ...
            <h1 class="text-primary font-semibold relative sm:text-lg text">{{ tv.name ? tv.name.substring(0, 13) + '...' : 'No description available' }}</h1>
            <div class="flex items-center mt-1 justify-between">
                <span class="text-xs text-gray-500 sm:text-sm">2023</span>
                <span class="text-xs text-gray-500 sm:text-sm">141 min</span>
                <span class="border-[1px] border-gray-500 text-xs text-gray-500 h-fit px-[2px] rounded-sm text-center sm:text-sm">tvshow</span>
            </div>
            </RouterLink>
        </div>
       </template>
