...
import getMovies from '../../composable/fetch'
    ...
    const { getTrendings, trendingsToday, baseImgUrl } = getMovies()
    onMounted(() => {
        getTrendings()
    })
</script>
<template>
    <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-2 p-2 max-w-screen-xl m-auto">
       <template v-for="trending in trendingsToday.results" :key="trending.id">
        <div class="min-h-[160px] w-full cursor-pointer">
            <RouterLink :to="{name : 'detail' , params: { id: trending.id } }" v-if="trending.media_type === 'movie'">
            <div class="relative flex flex-wrap items-center justify-center after:w-full after:h-full after:hover:bg-bgDark after:opacity-25 after:absolute after:z-30 after:top-0 after:ease-in after:duration-75">
                <img class="object-cover object-center" :src="`${baseImgUrl}${trending.poster_path}`" alt="">
                <div class="h-fit w-auto p-1 bg-primary text-xs rounded-full absolute top-1 right-1 font-semibold text-center">{{ trending.vote_average }}</div>
                <div class="absolute">
            ...
            <h1 class="text-primary font-semibold relative sm:text-lg text">
                {{ trending.title ? trending.title.substring(0, 13) + '...' : 'N/A' }}
            </h1>
            <div class="flex items-center mt-1 justify-between">
                <span class="text-xs text-gray-500 sm:text-sm">{{ trending.release_date ? trending.release_date.substring(0, 4) : 'N/A' }}</span>
                <span class="text-xs text-gray-500 sm:text-sm">141 min</span>
                <span class="border-[1px] border-gray-500 text-xs text-gray-500 h-fit px-[2px] rounded-sm text-center sm:text-sm">{{ trending.media_type === 'tv' ? 'tvshow' : 'movie' }}</span>
            </div>
            </RouterLink>
            <RouterLink :to="{name : 'tvdetail' , params: { id: trending.id } }" v-else-if="trending.media_type === 'tv'">
            <div class="relative flex flex-wrap items-center justify-center after:w-full after:h-full after:hover:bg-bgDark after:opacity-25 after:absolute after:z-30 after:top-0 after:ease-in after:duration-75">
                <img class="object-cover object-center" :src="`${baseImgUrl}${trending.poster_path}`" alt="">
                <div class="h-fit w-auto p-1 bg-primary text-xs rounded-full absolute top-1 right-1 font-semibold text-center">{{ trending.vote_average }}</div>
                <div class="absolute">
            ...
            <h1 class="text-primary font-semibold relative sm:text-lg text">
                {{ trending.name ? trending.name.substring(0, 13) + '...' : 'N/A' }}
            </h1>
            <div class="flex items-center mt-1 justify-between">
                <span class="text-xs text-gray-500 sm:text-sm">{{ trending.release_date ? trending.release_date.substring(0, 4) : 'N/A' }}</span>
                <span class="text-xs text-gray-500 sm:text-sm">141 min</span>
