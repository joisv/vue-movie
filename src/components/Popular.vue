...
import getMovies from '../composable/fetch'
    ...
    const { baseImgUrl, popularMovies, getPopularMovie } = getMovies()
    onMounted(() => {
        getPopularMovie()
    })
</script>
...
<template>
 <section>
    <div class="flex items-center gap-4 max-w-screen-xl m-auto">
        <div class="font-semibold text-primary text-2xl ml-2 mt-2 mb-3 sm:text-3xl lg:text-4xl sm:font-bold sm:opacity-80" title="New Movie">Popular</div>
    </div>
    <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-2 p-2 max-w-screen-xl m-auto">
        <template v-for="popular in popularMovies.results" :key="popular.id">
        <div class="min-h-[160px] w-full cursor-pointer">
            <RouterLink :to="{name : 'detail', params : { id: popular.id } }">
            <div class="relative flex flex-wrap items-center justify-center after:w-full after:h-full after:hover:bg-bgDark after:opacity-25 after:absolute after:z-30 after:top-0 after:ease-in after:duration-75">
                <img class="object-cover object-center" :src="`${baseImgUrl}${popular.poster_path}`" alt="">
                <div class="h-fit w-auto p-1 bg-primary text-xs rounded-full absolute top-1 right-1 font-medium text-center">{{ popular.vote_average }}</div>
                <div class="absolute">
            ...
            <h1 class="text-primary font-semibold relative sm:text-lg">{{popular.title ? popular.title.substring(0, 13) + '...' : 'N/A' }}</h1>
            <div class="flex items-center mt-1 justify-between">
                <span class="text-xs text-gray-500 sm:text-sm">{{ popular.release_date.substring(0, 4) }}</span>
                <span class="text-xs text-gray-500 sm:text-sm">114min</span>
